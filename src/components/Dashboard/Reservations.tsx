...
import { CheckCircle2, Trash2, RefreshCw, X, Loader2, Phone, Mail, User, Ticket } from "lucide-react";
...
export default function AdminReservations() {
  ...
  if (loading) return (
    <div className="p-4 md:p-8 flex items-center justify-center flex-col min-h-[500px] w-full">
      ...
      <p className="text-stone-500 font-medium tracking-wide animate-pulse">Loading reservations...</p>
    </div>
  );
  ...
  return (
    <div className="w-full h-full relative p-4 md:p-8 lg:p-12 overflow-y-auto bg-stone-50/30">
      ...
      <AnimatePresence>
        {confirmAction && (
          ...
          >
            ...
            >
              <div className="p-6">
                <h3 className="text-[19px] font-bold text-stone-900 mb-2 tracking-tight">{confirmAction.title}</h3>
                <p className="text-stone-500 text-[15px] leading-relaxed">{confirmAction.message}</p>
              </div>
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-8">
        <div>
          <h2 className="text-3xl md:text-4xl font-bold text-stone-800 tracking-tight">Event Reservations</h2>
          <p className="text-stone-500 mt-2 font-medium">Manage pending event tickets and digital dispatches</p>
        </div>
        <button 
      ...
      {reservations.length === 0 ? (
        <div className="bg-white rounded-2xl border border-stone-200/60 p-16 flex flex-col items-center justify-center shadow-sm">
          ...
          <h3 className="text-xl font-bold text-stone-800 mb-2">No Reservations Yet</h3>
          <p className="text-stone-500 max-w-sm text-center">When clients book tickets, their pending requests will elegantly appear right here.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {reservations.map((res) => (
            <div key={res.id} className="bg-white rounded-2xl border border-stone-200 shadow-sm hover:shadow-md hover:border-stone-300 transition-all overflow-hidden flex flex-col group relative">
              
              {/* Status Header Bar */}
              <div className="px-6 py-4 flex justify-between items-center border-b border-stone-100 bg-stone-50/50">
                <span className={`px-3 py-1 rounded-full text-[11px] uppercase tracking-wider font-bold shadow-sm flex items-center gap-1.5 ${
                  res.status === 'confirmed' ? 'bg-green-100 text-green-700 border border-green-200/60' :
                  res.status === 'pending' ? 'bg-amber-100 text-amber-700 border border-amber-200/60' :
                ...
                <span className="text-xs text-stone-400 font-medium">#{res.id.substring(0,6)}</span>
              </div>

              {/* Body */}
              <div className="p-6 flex-grow flex flex-col gap-4">
                <div>
                  <h3 className="text-lg font-bold text-stone-900 group-hover:text-[#8c9c74] transition-colors">{res.fullName}</h3>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 mt-2">
                  <div className="bg-stone-50 rounded-xl p-2.5 border border-stone-100 text-center flex flex-col justify-center">
                    <span className="block text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-0.5">Tickets</span>
                    <span className="block font-bold text-stone-800 text-[17px]">{res.tickets}</span>
                  </div>
                  <div className="bg-stone-50 rounded-xl p-2.5 border border-stone-100 text-center flex flex-col justify-center">
                    <span className="block text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-0.5">Childcare</span>
                    <span className="block font-bold text-stone-800 text-[17px]">{res.needsChildcare ? "Yes" : "No"}</span>
                  </div>
                  <div className="bg-green-50/40 rounded-xl p-2.5 border border-green-100/60 text-center flex flex-col justify-center">
                    <span className="block text-[10px] font-bold text-green-600/70 uppercase tracking-wider mb-0.5">Expected</span>
                    <span className="block font-bold text-green-700 text-[17px]">${Number(res.tickets) * 400}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="px-6 py-4 bg-stone-50/80 border-t border-stone-100 flex justify-end gap-2 items-center">
                ...
                {res.status !== 'cancelled' && (
                  ...
                  >
                    <X size={18} /> {res.status !== 'pending' && <span className="text-sm font-semibold">Cancel Ticket</span>}
                  </button>
                )}
                <button 
      ...
      <AnimatePresence>
        {updating && (
          ...
          >
            ...
            <span className="text-[14px] font-semibold tracking-wide">Processing...</span>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
